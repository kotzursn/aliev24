...
header("Content-Disposition: attachment; filename=export-groups.csv"); 
...
$r = mysqli_query($connect_to_db, "SELECT * FROM groups") or die(mysqli_error($connect_to_db));
...
$buffer = fopen(__DIR__ . '/export/groups.csv', 'w'); 
fputs($buffer, chr(0xEF) . chr(0xBB) . chr(0xBF));
foreach($prods as $val) { 
	fputcsv($buffer, $val, ';'); 
} 
